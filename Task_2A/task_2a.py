...
def detect_ArUco_details(image):
    ...
    * ArUco_corners = 
    ...
    if image is not None:
        ...
        corners, id, rejected = cv2.aruco.detectMarkers(image, aruco_dict, parameters=aruco_params)
        aruco_id = id.flatten()
        for id1 in range(len(aruco_id)):
            ar_id = int(aruco_id[id1])
            ar_corner = corners[id1].reshape(4,2)
            
            centerx = int((ar_corner[0][0] + ar_corner[2][0]) / 2)
            centery = int((ar_corner[0][1] + ar_corner[2][1]) / 2)
            
            angle = int(math.degrees(math.atan2(ar_corner[1][1] - ar_corner[0][1], ar_corner[1][0] - ar_corner[0][0])))
            
            ArUco_details_dict[ar_id] = [[centerx, centery], angle]
            ArUco_corners[ar_id] = ar_corner
    ##################################################
