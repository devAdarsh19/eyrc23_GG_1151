...
# Team ID:			[ GG_1151 ]
# Author List:		[ Pooja, Aayush Kumar, Madan Maskara, Adarsh Vinod ]
# Filename:			task_1a.py
...
def data_preprocessing(task_1a_dataframe):
	...
	#################	ADD YOUR CODE HERE	##################
	le = LabelEncoder()
	# converting string values to numerical data type
	gender_enc = le.fit_transform(task_1a_dataframe['Gender'])
	education_enc = le.fit_transform(task_1a_dataframe['Education'])
	joinYear_enc = le.fit_transform(task_1a_dataframe['JoiningYear'])
	city_enc = le.fit_transform(task_1a_dataframe['City'])
	everBenched_enc = le.fit_transform(task_1a_dataframe['EverBenched'])
	
	# replacing older data with new numerical data
	task_1a_dataframe['Education'] = education_enc
	task_1a_dataframe['City'] = city_enc
	task_1a_dataframe['JoiningYear'] = joinYear_enc
	task_1a_dataframe['Gender'] = gender_enc
	task_1a_dataframe['EverBenched'] = everBenched_enc
	
	encoded_dataframe = task_1a_dataframe.fillna(task_1a_dataframe.median()) 		# filling null values with mean of the column
...
def identify_features_and_targets(encoded_dataframe):
	...
	#################	ADD YOUR CODE HERE	##################
	features = encoded_dataframe.loc[ : ,['Education', 'JoiningYear', 'City', 'PaymentTier', 'Age', 'Gender', 'EverBenched', 'ExperienceInCurrentDomain']]
...
def load_as_tensors(features_and_targets):
	...
	#################	ADD YOUR CODE HERE	##################
	#Converting each field to tensor
	education_T = torch.tensor(features_and_targets[0].Education, dtype=torch.float32)
	joinYear_T = torch.tensor(features_and_targets[0].JoiningYear, dtype=torch.float32)
	city_T = torch.tensor(features_and_targets[0].City, dtype=torch.float32)
	paymentTier_T = torch.tensor(features_and_targets[0].PaymentTier, dtype=torch.float32)
	age_T = torch.tensor(features_and_targets[0].Age, dtype=torch.float32)
	gender_T = torch.tensor(features_and_targets[0].Gender, dtype=torch.float32)
	everBenched_T = torch.tensor(features_and_targets[0].EverBenched, dtype=torch.float32)
	experience_T = torch.tensor(features_and_targets[0].ExperienceInCurrentDomain, dtype=torch.float32)
	
	#features tensor
	features_T = torch.stack([education_T, joinYear_T, city_T, paymentTier_T, age_T, gender_T, everBenched_T, experience_T], dim=1)
	
	target_T = torch.tensor(features_and_targets[1], dtype=torch.float32, requires_grad=True)
	
	#splitting into training and testing data
	train_split = int(len(features_T) * 0.75)
	X_train_tensor, y_train_tensor = features_T[:train_split], target_T[:train_split]
	X_test_tensor, y_test_tensor = features_T[train_split:], target_T[train_split:]
 
	
	#Creating iterable dataset object
...
def training_function(model, number_of_epochs, tensors_and_iterable_training_data, loss_function, optimizer):
	...
	for epoch in range(number_of_epochs):
		optimizer.zero_grad()
		output = model(tensors_and_iterable_training_data[0])
  
		y_train_reshaped = tensors_and_iterable_training_data[2].view(-1,1)
  
		loss = loss_function(output, y_train_reshaped)
		loss.backward()
		optimizer.step()
...
def validation_function(trained_model, tensors_and_iterable_training_data):
	...
	#################	ADD YOUR CODE HERE	##################
	with torch.no_grad():
		output = trained_model(tensors_and_iterable_training_data[1])
		prediction = (output >= 0.5).int()
		correct_pred = (prediction == tensors_and_iterable_training_data[3].view(-1,1)).sum().item()

		total_samples = len(tensors_and_iterable_training_data[3])
		model_accuracy = (correct_pred/total_samples)*100
	##########################################################
